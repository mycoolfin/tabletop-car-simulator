...
CALIBRATE_PHRASE = "calibrate".encode('utf-8')
IDENTIFY_PHRASE = "identify".encode('utf-8')
...
class Vision():
    ...
    def confirm_calibrated(self):
        corners = parse_json(self.client.send_message(CALIBRATE_PHRASE))
        if corners is not None:
            print(msgHeader + "Received calibration confirmation from the tracker.")
        return corners

    def confirm_identified(self):
        numCarsFound = parse_json(self.client.send_message(IDENTIFY_PHRASE))
        if numCarsFound is not None:
            print(msgHeader + "Received identification confirmation from the tracker.")
        return numCarsFound
